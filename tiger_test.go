...
func TestTiger(T *testing.T) {
	...
	for i, vec := range []testVector{
		...
		testVector{"ABCDEFGHIJKLMNOPQRSTUVWXYZ=abcdefghijklmnopqrstuvwxyz+0123456789",
			"48ceeb6308b87d46e95d656112cdf18d97915f9765658957"},
		testVector{"Tiger - A Fast New Hash Function, by Ross Anderson and Eli Biham",
			"8a866829040a410c729ad23f5ada711603b3cdd357e4c15e"},
		testVector{"Tiger - A Fast New Hash Function, by Ross Anderson and Eli Biham, proceedings of Fast Software Encryption 3, Cambridge.",
			"ce55a6afd591f5ebac547ff84f89227f9331dab0b611c889"},
		testVector{"Tiger - A Fast New Hash Function, by Ross Anderson and Eli Biham, proceedings of Fast Software Encryption 3, Cambridge, 1996.",
			"631abdd103eb9a3d245b6dfd4d77b257fc7439501d1568dd"},
		testVector{"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-",
...
func TestTiger_sum(T *testing.T) {
	...
	for i, vec := range []testVector{
		testVector{"Tiger - A Fast New Hash Function, by Ross Anderson and Eli Biham",
			"8a866829040a410c729ad23f5ada711603b3cdd357e4c15e"},
		testVector{", proceedings of Fast Software Encryption 3, Cambridge.",
			"ce55a6afd591f5ebac547ff84f89227f9331dab0b611c889"},
	} {
		io.Copy(t, strings.NewReader(vec.data))
		switch sum := hex.EncodeToString(t.Sum(nil)); sum {
